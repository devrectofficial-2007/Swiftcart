...
class ProductService {
  ...
  Future<String> uploadProductImage(File imageFile, String productId, int imageIndex) async {
    try {
      final ref = _storage.ref().child('products').child(productId).child('image_$imageIndex.jpg');
  ...
  Future<String> addProduct(Map<String, dynamic> productData, List<File> imageFiles) async {
    try {
      ...
      for (int i = 0; i < imageFiles.length; i++) {
        final imageUrl = await uploadProductImage(imageFiles[i], productId, i);
  ...
  Future<void> updateProduct(String productId, Map<String, dynamic> productData, List<File>? newImageFiles, List<String>? existingImages) async {
    try {
      ...
      if (newImageFiles != null && newImageFiles.isNotEmpty) {
        ...
        for (int i = 0; i < newImageFiles.length; i++) {
          final imageUrl = await uploadProductImage(newImageFiles[i], productId, i);
          imageUrls.add(imageUrl);
        }
        ...
        updateData['faceImage'] = existingImages.isNotEmpty ? existingImages[0] : '';
      }
  ...
  Future<void> deleteProduct(String productId) async {
    try {
      ...
      try {
        final productImagesRef = _storage.ref().child('products').child(productId);
        await productImagesRef.listAll().then((result) {
  ...
  Future<List<Map<String, dynamic>>> getAllProducts() async {
    try {
      ...
      final Map<dynamic, dynamic> productsData = snapshot.value as Map<dynamic, dynamic>;
  ...
  Future<Map<String, dynamic>?> getProductById(String productId) async {
    try {
      final DataSnapshot snapshot = await _database.child('products').child(productId).get();
  ...
  Future<List<Map<String, dynamic>>> getProductsBySubcategory(String categoryId, String subcategoryId) async {
    try {
      final allProducts = await getAllProducts();
